...
def check_product_status(backend, product_ids=[], product_groups=[], host_group_ids=[], depot_ids=[], exclude=[], verbose=False, strict=False):  # pylint: disable=line-too-long, dangerous-default-value, too-many-arguments, too-many-locals, too-many-branches, too-many-statements
	...
	for depot_id in depot_ids:
		...
		for poc in poducts_on_client:  # pylint: disable=protected-access, line-too-long
			...
			if (
				poc.productVersion != product_on_depot["productVersion"] or
	...
	for depot_id in depot_ids:
		if (
			depot_id in action_request_on_client or
			depot_id in product_problems_on_client or
			depot_id in product_version_problems_on_client or
			missing_products.get(depot_id)
		):
