...
class RPCConfigStateMixin(Protocol):
	...
	def _configState_to_objects_with_host_parameter_audit_candidates(
	...
	def _configState_audit_host_parameter_value_set(self: BackendProtocol, config_states: list[ConfigState]) -> None:
		if not config_states:
			return

		if not audit_log_event_enabled(AuditLogEventType.HOST_PARAMETER_VALUE_SET):
			return
		...
		server_ids = set(self.host_getIdents(returnType="str", type="OpsiConfigserver", id=object_ids))
		depot_ids = set(self.host_getIdents(returnType="str", type="OpsiDepotserver", id=object_ids)) - server_ids
		...
		try:
			...
			logger.error("Failed to write HostParameter ConfigState audit log: %s", err, exc_info=True)
	...
	def configState_insertObject(self: BackendProtocol, configState: dict | ConfigState) -> None:
		...
		config_states, audit_candidates = self._configState_to_objects_with_host_parameter_audit_candidates(configState)
		...
		if audit_candidates:
			self._configState_audit_host_parameter_value_set(audit_candidates)
	...
	def configState_createObjects(self: BackendProtocol, configStates: list[dict] | list[ConfigState] | dict | ConfigState) -> None:
		...
		configStates, audit_candidates = self._configState_to_objects_with_host_parameter_audit_candidates(configStates)
