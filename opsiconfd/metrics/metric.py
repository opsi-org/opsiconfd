...
class Metric:
	...
	def redis_key_prefix(self) -> str:
		if not self._redis_key_prefix:
			from opsiconfd.config import config

			self.redis_key_prefix = f"{config.redis_key('stats')}:{self.id}"

		assert self._redis_key_prefix
		return self._redis_key_prefix

	@redis_key_prefix.setter
	def redis_key_prefix(self, redis_prefix: str) -> None:
	...
	def redis_key(self) -> str:
		if not self._redis_key:
			self._redis_key = self.redis_key_prefix
			for var in self.vars:
				self._redis_key += ":{" + var + "}"
		assert self._redis_key
