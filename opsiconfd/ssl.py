...
from .backend import get_server_role, get_unrestricted_backend
from .config import (
...
def depotserver_setup_ca() -> bool:
	...
	ca_crt = load_certificate(FILETYPE_PEM, get_unrestricted_backend().getOpsiCACert())  # pylint: disable=no-member
		...
		if server_role == "config":
			...
			for attempt in (1, 2, 3, 4, 5):
				try:  # pylint: disable=loop-try-except-usage
					...
					pem = get_unrestricted_backend().host_getTLSCertificate(server_cn)  # pylint: disable=no-member,loop-invariant-statement
