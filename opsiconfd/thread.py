...
BASE_ABOVE_PY_37 = 'BrokenExecutor' in dir(_base)
QUEUE_ABOVE_PY_37 = 'SipleQueue' in dir(queue)
...
def _python_exit():
	global _shutdown  # pylint: disable=invalid-name,global-statement
	_shutdown = True
	items = list(_threads_queues.items())
	for thread, _queue in items:
		_queue.put(None)
	for thread, _queue in items:
		thread.join()
...
class _WorkItem:  # pylint: disable=too-few-public-methods
...
class _CustomThread(threading.Thread):
	...
	def __init__(self , name , executor_reference, work_queue, initializer, initargs , thread_counter):  # pylint: disable=too-many-arguments
	...
	def run(self):
		...
		try:
			self._executor._adjust_free_thread_count(1)  # pylint: disable=protected-access
			while True:
				try:
					work_item = self._work_queue.get(block=True , timeout = self._executor._keep_alive_time)  # pylint: disable=protected-access
				except queue.Empty:
					...
					# but no worker takes it out in the meantime. The last task will never run if there's no new tasks trigger
					# ThreadPoolExecutor._adjust_thread_count afterwards.
					...
					if self._thread_count_id < self._executor._min_workers:  # pylint: disable=protected-access
						continue

					with self._executor._free_thread_count_lock:  # pylint: disable=protected-access
						if self._executor._free_thread_count > self._executor._min_workers:  # pylint: disable=protected-access
				...
				if _shutdown or executor is None or executor._shutdown:  # pylint: disable=protected-access
...
# Upward Compatible
_class_brokenexecutor = _base.BrokenExecutor if BASE_ABOVE_PY_37 else BrokenExecutor  # pylint: disable=invalid-name
...
class ThreadPoolExecutor(_base.Executor):  # pylint: disable=too-many-instance-attributes
	...
	def __init__(self, max_workers=None, thread_name_prefix='',
		...
		if 1 <= max_workers < self._min_workers:
	...
	def _adjust_thread_count(self):
		...
		# the worker threads.
		def weakref_cb(_, _queue=self._work_queue):
			_queue.put(None)
		...
		with self._free_thread_count_lock:
			_tflag = self._free_thread_count < self._min_workers

		if _tflag and num_threads < self._max_workers:
			thread_name = '%s_%d' % (self._thread_name_prefix or self,
									 num_threads)
			thread = _CustomThread(name=thread_name,
