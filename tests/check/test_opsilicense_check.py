...
@pytest.mark.parametrize(
	...
	(
		("custom_ca", [], {"ssl_server_cert_type": "opsi-ca"}, "ok"),
		("letsencrypt", [], {"ssl_server_cert_type": "opsi-ca"}, "ok"),
		("custom_ca", [], {"ssl_server_cert_type": "custom-ca"}, "error"),
		("letsencrypt", [], {"ssl_server_cert_type": "letsencrypt"}, "error"),
		("sso", [], {"saml-idp-entity-id": ""}, "ok"),
		("sso", [], {"saml-idp-entity-id": "https://keycloak.opsi.test/realms/master"}, "error"),
		("scalability1", [], {"workers": 1}, "ok"),
		("scalability1", [], {"workers": 2}, "error"),
		("directory-connector", [{"category": "ldap_auth", "config": "ldap_url", "value": ""}], {}, "ok"),
		("directory-connector", [{"category": "ldap_auth", "config": "ldap_url", "value": "ldaps:///ldap"}], {}, "error"),
	),
)
def test_check_licenses_missing(
	...
	def mock_licensing_info(
		...
		res["available_modules"] = [m for m in res["available_modules"] if m != missing_module_id]
		return res
	...
	with mock.patch("opsiconfd.backend.rpc.general.RPCGeneralMixin.backend_getLicensingInfo", mock_licensing_info):
		with get_opsi_config(opsi_config), get_config(opsiconfd_config):
			...
			if expected_status == "error":
				assert f"module '{missing_module_id}' is not available." in partial_result.message
			else:
