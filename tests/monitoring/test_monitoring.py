	...
	with mysql.connection():
		with mysql.session() as session:
			...
			if res == 0:
				...
				session.execute(
					f'INSERT INTO CONFIG_VALUE (configId, value, isDefault) VALUES ("clientconfig.depot.id", "{get_depotserver_id()}", 1);'
				)
...
def test_check_disk_usage(
	...
	assert expected_result == json.loads(result.body)  # ty: ignore[invalid-argument-type]
...
def test_check_disk_usage_no_result(
	...
	assert json.loads(result.body) == {  # ty: ignore[invalid-argument-type]
...
def test_check_short_product_status(
	...
	assert json.loads(result.body) == expected_result  # ty: ignore[invalid-argument-type]
