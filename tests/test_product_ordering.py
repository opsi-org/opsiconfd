	...
	with (
		...
		for idx, product in enumerate(products):
			if idx in (3, 23, 45, 77, 89):
				...
				threads.append(threading.Thread(target=delete_products_jsonrpc, args=(test_client, "", [products[idx]])))
			else:
...
def test_renew_cache_on_create_products(test_client):  # pylint: disable=redefined-outer-name
	...
	with (get_config({"jsonrpc_time_to_cache": 0}), depot_jsonrpc(test_client, "", depot_id), sync_redis_client() as redis):
